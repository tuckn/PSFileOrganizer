using module "..\Modules\RenameFileWithDateCode.psm1"
...
if ((Get-Item -LiteralPath $FilePath).PSIsContainer) {
    ...
    foreach ($f in Get-ChildItem $childPath) {
        try {
            Rename-FileWithDateCode -FilePath $f.FullName -DateFormat $DateFormat -OriginalName $OriginalName
        }
        catch {
            Write-Error $_
        }
...
elseif (Test-Path -LiteralPath $FilePath) {
    try {
        Rename-FileWithDateCode -FilePath $FilePath -DateFormat $DateFormat -OriginalName $OriginalName
    }
    catch {
        Write-Error $_
    }
