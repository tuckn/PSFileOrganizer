using module "..\Modules\EditFileCreationTimeFromName.psm1"
...
if ((Get-Item -LiteralPath $FilePath).PSIsContainer) {
    ...
    foreach ($f in Get-ChildItem $childPath) {
        try {
            Edit-FileCreationTimeFromName -FilePath $f.FullName
        }
        catch {
            Write-Error $_
        }
...
elseif (Test-Path -LiteralPath $FilePath) {
    try {
        Edit-FileCreationTimeFromName -FilePath $FilePath
    }
    catch {
        Write-Error $_
    }
